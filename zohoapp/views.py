...
def recurring_bill(request):
    ...
    for r in recur:
        ...
        r['cust_name'] = " ".join(cn)  
        v_email = vendor_table.objects.filter(first_name = vn[0],last_name = vn[1]).first() 
        record = {"start_date":str(r['start_date']),"bill_no":str(r['bill_no']),"profile_name":r['profile_name'],"vend_name":str(r['vend_name']),"cust_name":r['cust_name'],"grand_total":r['grand_total'],"status":r['status'],"balance":str(r['balance']),"id":r['id'],"v_email":str(v_email.vendor_email)}
...
def view_bills(request):
    ...
    pickup_records = []
    for bill in bills:
        record = {"bill_date":str(bill.bill_date),"bill_no":str(bill.bill_no),"customer_name":bill.customer_name,"vendor_name":bill.vendor_name,"vendor_email":str(bill.vendor_email),"status":bill.status,"total":bill.total,"id":bill.id,"balance":str(bill.balance)}
        pickup_records.append(record)
